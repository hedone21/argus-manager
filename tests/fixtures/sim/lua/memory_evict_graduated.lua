...
--   ctx.pressure.memory (0.0~1.0): 메모리 압박 수치
--     ≥ 0.7 → kv_evict_sliding(budget=0.5)  — 강력한 eviction
--     ≥ 0.4 → kv_evict_sliding(budget=0.8)  — 완만한 eviction
--
-- ctx.trigger.mem_low: bool — 메모리 부족 트리거
...
function decide(ctx)
  local cmds = {}
  local pressure_mem = ctx.pressure and ctx.pressure.memory or 0.0
  local mem_low      = ctx.trigger and ctx.trigger.mem_low  or false
